...
#define NUMPMT 20 // 16 //20

#define INTERVALMDRF 0.2 // 0.25
...
void searchTree(node *root, float *event){
    int idx=root->idx;
    if(root->leaf==-1){ // judge whether it is a internal node
        float boundary=root->boundary;
        if(event[idx]<boundary){
            float test=event[idx]; // a variable for testing, just ignore
            searchTree(root->leftChild, event); // search the left branch
            if(budgetTotal>boundary-event[idx]){ // if the accumulated error "budgetTotal" is larger than the euclidean distance to the boundary, also search the other branch
                searchTree(root->rightChild, event);
            }
        }else{
            float test=event[idx]; // a variable for testing, just ignore
            searchTree(root->rightChild, event); // search the right branch
            if(budgetTotal>event[idx]-boundary){ // if the accumulated error "budgetTotal" is larger than the euclidean distance to the boudary, also search the other branch
                searchTree(root->leftChild, event);
            }
        }
    }else{
        ...
        temp=sqrt(temp); // calculate the euclidean distance
...
int main(){
    int flag, size, i, j;
    float x, y, *MDRFRead, *PMTSignals, *event, total;
    node *root;
    FILE *fid;
    ...
    fid = fopen("E:\\research\\data\\20180314_cnn\\MDRFSimulated_252X252_NumPhoton200000_NumSiPMs20_CsI_Barrier1_QE0.2_lambertianRef0.9_gelIndex1.500_WholeLargeRodSpace1_.bin", "rb");
    ...
    fid = fopen("E:\\research\\data\\20180205\\7lineSimulated.bin", "rb"); // the test data, detector responses by illuminating the detector with 5 gamma-ray slit beams.
    ...
    fid = fopen("E:\\research\\data\\20180508_7lineTrueLable\\resultKDTree.bin", "wb");
    ...
    for(i=0;i<size;i++){
        ...
        searchTree(root, event); // search for the query vector "event"
		...
		budgetTotal = 1.0 / pow(budgetTotal * total, 2);
